...
public class GuildChannelsRepository(
    ...
    AmnyamDbContext mlkAdminDbContext) : IGuildChannelsRepository
{
    ...
    public async Task UpsertDbTextChannelAsync(GuildChannel channel)
    {
        try
        {
    ...
    public async Task RemoveDbTextChannelAsync(ulong id)
    {
        try
        {
            await RemoveEntityAsync<GuildChannel>(
                x => x.DiscordId == id);
